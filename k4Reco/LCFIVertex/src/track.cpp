...
Track::Track(Event* Event, const HelixRep& H, const Vector3& Momentum, const double& cha, const SymMatrix5x5& cov,
             std::vector<int> hits, void* trackNum)
    : _Event(Event), _H(H), _PMomentum(Momentum), _Charge(cha), _CovarianceMatrix(cov), _NumHitsSubDetector(hits),
      _TrackingNum(trackNum) {}

Event* Track::event() const { return _Event; }
...
const HelixRep& Track::helixRep() const { return _H; }

HelixRep& Track::helixRep() { return _H; }
double Track::charge() const { return _Charge; }

const Vector3& Track::momentum() const { return _PMomentum; }

const SymMatrix5x5& Track::covarianceMatrix() const { return _CovarianceMatrix; }
...
double Track::significance(Projection Proj) const {
  ...
  case RPhi: {
    ...
    double ErrorIP = (IPErr(x, x) * pow(POCAVector.x(), 2.0) + 2.0 * IPErr(x, y) * POCAVector.x() * POCAVector.y() +
                      IPErr(y, y) * pow(POCAVector.y(), 2.0)) /
                     POCAVector.mag2(RPhi);
    ...
    // std::cout <<  POCAVector.mag(RPhi)*1000.0 << " " << sqrt(ErrorIP)*1000.0 << " " << sqrt(ErrorTrack)*1000.0 << " "
    // << POCAVector.mag(RPhi)/sqrt(ErrorIP+ErrorTrack) << " ";
  ...
  case ThreeD: {
    ...
    // double ErrorIP = (IPErr(x,x)*pow(POCAVector.x(),2.0) + 2*IPErr(x,y)*POCAVector.x()*POCAVector.y() +
    // IPErr(y,y)*pow(POCAVector.y(),2.0)  + IPErr(z,z)*pow(POCAVector.z(),2.0)+
    // 2*IPErr(x,z)*POCAVector.x()*POCAVector.z() +2*IPErr(z,y)*POCAVector.z()*POCAVector.y())/ POCAVector.mag2(ThreeD)
    // ; this should probably be only 0,0+3,3 given the definition of the POCAVector in the Rphi case. ignore cross
    // terms?
    double ErrorTrack =
        this->covarianceMatrix()(0, 0) + this->covarianceMatrix()(3, 3) + 2 * this->covarianceMatrix()(0, 3);
...
double Track::signedSignificance(Projection Proj, Jet* MyJet) const {
  ...
  Vector3 POCAVector = track->position() - this->event()->interactionPoint();
  switch (Proj) {
  case RPhi: { // Sign the significances relative to the jet so if the track and jet cross in front of IP the
  ...
  case Z: {
    ...
    double TanlambdaJet =
        MyJet->momentum().z() / sqrt(pow(MyJet->momentum().x(), 2.0) + pow(MyJet->momentum().y(), 2.0));
...
void* Track::trackingNum() const { return _TrackingNum; }
} // namespace vertex_lcfi
