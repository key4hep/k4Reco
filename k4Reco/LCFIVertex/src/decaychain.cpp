...
DecayChain::DecayChain(const DecayChain& OldDecayChain)
    : _OwnerJet(OldDecayChain.jet()), _AttachedTracks(OldDecayChain.attachedTracks()),
      _Vertices(OldDecayChain.vertices()) {
...
const std::vector<Track*>& DecayChain::allTracks() const {
  ...
  if (_AllTracksValid) { /*no op*/
  } else {
    _AllTracks.clear();
    for (std::vector<Track*>::const_iterator iTrack = _AttachedTracks.begin(); iTrack != _AttachedTracks.end();
         ++iTrack) {
      _AllTracks.push_back(*iTrack);
    }

    for (std::vector<Vertex*>::const_iterator iVertex = _Vertices.begin(); iVertex != _Vertices.end(); ++iVertex) {
      for (std::vector<Track*>::const_iterator iTrack = (*iVertex)->tracks().begin();
           iTrack != (*iVertex)->tracks().end(); ++iTrack)
        _AllTracks.push_back(*iTrack);
    }
...
const std::vector<Track*>& DecayChain::attachedTracks() const { return _AttachedTracks; }

const std::vector<Vertex*>& DecayChain::vertices() const { return _Vertices; }

double DecayChain::charge() const {
  // Chached variable check the status of the cache
  if (_ChargeValid) { /*no op*/
  } else {
    _Charge = 0.0;
    ...
    for (std::vector<Track*>::const_iterator iTrack = allTracks.begin(); iTrack != allTracks.end(); ++iTrack) {
      ...
      if (numVertsWithTrack < 2)
        _Charge += (*iTrack)->charge();
    }
  }
  return _Charge;
}
