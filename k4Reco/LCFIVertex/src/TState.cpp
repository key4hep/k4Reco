...
TState::TState(TrackState* TrackState) {
  ...
  fB = lcfiplus::Globals::Instance()->getBField();

  double sinp = sin(helix.phi());
  double cosp = cos(helix.phi());
  double d0 = helix.d0();
  double z0 = helix.z0();
  double invR = helix.invR();
...
void TState::GetMeasurement(const double xyz[], double m[], double V[]) const {
  ...
  double sigmaS =
      0.1 + 10. * sqrt((d[0] * d[0] + d[1] * d[1] + d[2] * d[2]) / (m[3] * m[3] + m[4] * m[4] + m[5] * m[5]));
  double h[6];
  ...
  if (true) {
    double mVv[6] = {V[0] + h[0] * h[0], V[1] + h[0] * h[1], V[2] + h[1] * h[1],
                     V[3] + h[0] * h[2], V[4] + h[1] * h[2], V[5] + h[2] * h[2]};

    double mVvp[9] = {V[6] + h[0] * h[3],  V[7] + h[1] * h[3],  V[8] + h[2] * h[3],
                      V[10] + h[0] * h[4], V[11] + h[1] * h[4], V[12] + h[2] * h[4],
                      V[15] + h[0] * h[5], V[16] + h[1] * h[5], V[17] + h[2] * h[5]};

    double mS[6] = {mVv[2] * mVv[5] - mVv[4] * mVv[4], mVv[3] * mVv[4] - mVv[1] * mVv[5],
                    mVv[0] * mVv[5] - mVv[3] * mVv[3], mVv[1] * mVv[4] - mVv[2] * mVv[3],
                    mVv[1] * mVv[3] - mVv[0] * mVv[4], mVv[0] * mVv[2] - mVv[1] * mVv[1]};

    double s = (mVv[0] * mS[0] + mVv[1] * mS[1] + mVv[3] * mS[3]);
    s = (s > 1.E-20) ? 1. / s : 0;

    double mSz[3] = {mS[0] * d[0] + mS[1] * d[1] + mS[3] * d[2], mS[1] * d[0] + mS[2] * d[1] + mS[4] * d[2],
...
void TState::TransportBz(double S, double P[], double C[]) const {
  ...
  double mJ[6][6] = {{1, 0, 0, sB, cB, 0}, {0, 1, 0, -cB, sB, 0}, {0, 0, 1, 0, 0, S},
...
bool TState::GetDStoTStateBz(const TState* p, double& DS, double& DS1) const {
  ...
  for (int j = 0; j < 2; j++) {
    for (int j1 = 0; j1 < 2; j1++) {
      bool flag = fabs(g[j][0]) < 60. && fabs(g1[j1][0]) < 60. && fabs(g[j][1]) < 60. && fabs(g1[j1][1]) < 60. &&
                  fabs(g[j][2]) < 125. && fabs(g1[j1][2]) < 125. && (fabs(ss[j]) + fabs(ss1[j1])) < 60.;
      if (!flag)
        continue;
