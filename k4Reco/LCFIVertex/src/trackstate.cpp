...
TrackState::TrackState(const HelixRep& init, const double& cha, const SymMatrix5x5& cov, Track* const tra)
    : _Charge(cha), _Init(init), _InitCovarMatrix(cov), _PosValid(0), _MomValid(0), _PositionCovarValid(0),
      _InversePositionCovarValid(0), _ParentTrack(tra) {
  // TODO Check for invr==0?
}
...
void TrackState::swimToStateNearest(const Vector3& Point) {
  ...
  if (this->isCharged()) {
    ...
    if (this->distanceTo(Point) < _swimprecision)
      return;
    ...
    do {
      ...
      double c = (-2 * (-(invrval * _Init.tanLambda() * (_Init.z0() - P.z() + s0 * _Init.tanLambda())) +
                        invrval * P.x() * cos(_Init.phi() + invrval * s0) +
                        (-1.0 + invrval * -_Init.d0()) * sin(invrval * s0) +
                        invrval * P.y() * sin(_Init.phi() + invrval * s0))) /
                 invrval;
      double b =
          2 * (pow(_Init.tanLambda(), 2) + (1.0 - invrval * -_Init.d0()) * cos(invrval * s0) -
               invrval * P.y() * cos(_Init.phi() + invrval * s0) + invrval * P.x() * sin(_Init.phi() + invrval * s0));
      double a = invrval * (invrval * P.x() * cos(_Init.phi() + invrval * s0) +
                            (-1.0 + invrval * -_Init.d0()) * sin(invrval * s0) +
                            invrval * P.y() * sin(_Init.phi() + invrval * s0));
...
void TrackState::swimToStateNearest(TrackState* const TrackToSwimTo) {
  if (this->isNeutral() && TrackToSwimTo->isNeutral()) {
    ...
    p43.y() = p4.y() - p3.y();
    p43.z() = p4.z() - p3.z();
    // TODO should the user know about this? Track isn't swum cos its either parallell or coincident with the other
    // track.
    if (fabs(p43.x()) < std::numeric_limits<double>::epsilon() &&
        fabs(p43.y()) < std::numeric_limits<double>::epsilon() &&
        fabs(p43.z()) < std::numeric_limits<double>::epsilon())
      return;
    p21.x() = p2.x() - p1.x();
    p21.y() = p2.y() - p1.y();
    p21.z() = p2.z() - p1.z();
    if (fabs(p21.x()) < std::numeric_limits<double>::epsilon() &&
        fabs(p21.y()) < std::numeric_limits<double>::epsilon() &&
        fabs(p21.z()) < std::numeric_limits<double>::epsilon())
      return;
  ...
  if ((this->isCharged() && TrackToSwimTo->isCharged()) || (this->isCharged() && TrackToSwimTo->isNeutral()) ||
      (this->isNeutral() && TrackToSwimTo->isCharged())) {
    ...
    do {
      do {
        ...
        // If the step we are going to take takes us uphill then we have found a minimum so break the inner loop and go
        // to higher precision
        toswimto.swimToStateNearest(this->position());
...
void TrackState::swimToStateNearestXY(const Vector3& Point) {
  ...
  if (this->isCharged()) {
    ...
    this->resetToRef();
    if (this->xyDistanceTo(Point) < _swimprecision)
      return;

    // So first find center of the circle in the XY plane
    Vector3 center = Vector3(this->position().x() + (sin(_Init.phi()) / _Init.invR()),
                             this->position().y() - (cos(_Init.phi()) / _Init.invR()), 0);
    ...
    // Thats one of two possible minima, check which one we want, we reset earlier so we're swimming relative to the
    // refpoint
...
double TrackState::chi2(const Vector3& Point) {
  ...
  if (this->distanceTo(Point) <= Residual(0)) {
    if (fabs(this->distanceTo(Point) - Residual(0)) < _swimprecision) {
      ...
      std::cerr << "Warning trackstate.cpp:364:chi2 2D Distance to track was longer than 3D - swimming problem?"
                << std::endl;
    }
  ...
  if (std::isnan(chi2)) {
    ...
    std::cerr << "Chi2: " << prec_inner_prod(trans(Residual), prec_prod(this->inversePositionCovarMatrix(), Residual))
              << std::endl
              << std::endl;
  }
...
double TrackState::chi2_nomove(const Vector3& Point) {
  ...
  if (this->distanceTo(Point) <= Residual(0))
    ...
    // std::cout << "Chi2: " << prec_inner_prod(trans(Residual),prec_prod(this->inversePositionCovarMatrix(),
    // Residual))<< std::endl<< std::endl;
  }
...
const Vector3& TrackState::position() const {
  if (!_PosValid) {
    if (this->isCharged()) {
      ...
      _Position.x() =
          -_Init.d0() * sin(phival) + (sin(phival) - sin(phival - _Init.invR() * _DistanceSwum)) / _Init.invR();
      // 		-d0        *Sin(phi)    + (Sin(phi)   -Sin(phi   -invr        *s            ))/invr
      _Position.y() =
          _Init.d0() * cos(phival) + (-cos(phival) + cos(phival - _Init.invR() * _DistanceSwum)) / _Init.invR();
      //		d0        *Cos(phi)    + (-Cos(phi)   +Cos(phi   -invr        *s            ))/invr
      //_Position.x() = (_Init.d0()*sin(phival)) +
      //((sin(phival)-sin(phival-(_DistanceSwum*(-_Init.invR()))))/(-_Init.invR())); _Position.y() =
      //(-_Init.d0()*cos(phival)) + ((cos(phival-(_DistanceSwum*(-_Init.invR())))-cos(phival))/(-_Init.invR()));
      _Position.z() = (_DistanceSwum * _Init.tanLambda()) + _Init.z0();
      // std::cout << "Cswim " << _DistanceSwum << " to " <<_Position << std::endl;

      //_Position.x() = (_Init.d0()*sin(phival)) +
      //((sin(phival)-sin(phival-(_DistanceSwum*_Init.invR())))/_Init.invR()); _Position.y() = (-_Init.d0()*cos(phival))
      //+ ((cos(phival-(_DistanceSwum*_Init.invR()))-cos(phival))/_Init.invR()); _Position.z() =
      //(_DistanceSwum/tan(_Init.theta())) + _Init.z0();
    }
...
const Matrix3x3 TrackState::vertexErrorContribution(Vector3 point) const {
  ...
  // std::cout << "*";std::cout.flush();
  double EXYZ[6];
  EXYZ[0] = pow(DXDD, 2) * this->positionCovarMatrix()(0, 0) + 2.0 * DXDD * DXDZ * this->positionCovarMatrix()(0, 1) +
            pow(DXDZ, 2) * this->positionCovarMatrix()(1, 1);
  EXYZ[1] = DXDD * DYDD * this->positionCovarMatrix()(0, 0) +
            (DXDD * DYDZ + DXDZ * DYDD) * this->positionCovarMatrix()(0, 1) +
            DXDZ * DYDZ * this->positionCovarMatrix()(1, 1);
  EXYZ[2] = pow(DYDD, 2) * this->positionCovarMatrix()(0, 0) + 2.0 * DYDD * DYDZ * this->positionCovarMatrix()(0, 1) +
            pow(DYDZ, 2) * this->positionCovarMatrix()(1, 1);
  ...
  SymMatrix3x3 GG;
  GG.clear();
  GG(0, 0) = GG(0, 0) + DETEUV * (ROTAT(0, 0) * ROTAT(0, 0) * EUV[2] - 1.0 * ROTAT(0, 0) * ROTAT(1, 0) * EUV[1] +
                                  ROTAT(1, 0) * ROTAT(1, 0) * EUV[0]);
  GG(1, 0) = GG(1, 0) + DETEUV * (ROTAT(0, 0) * ROTAT(0, 1) * EUV[2] - ROTAT(0, 0) * ROTAT(1, 1) * EUV[1] -
                                  ROTAT(1, 0) * ROTAT(0, 1) * EUV[1] + ROTAT(1, 0) * ROTAT(1, 1) * EUV[0]);
  GG(1, 1) = GG(1, 1) + DETEUV * (ROTAT(0, 1) * ROTAT(0, 1) * EUV[2] - 1.0 * ROTAT(0, 1) * ROTAT(1, 1) * EUV[1] +
                                  ROTAT(1, 1) * ROTAT(1, 1) * EUV[0]);
  GG(2, 0) = GG(2, 0) + DETEUV * (ROTAT(0, 0) * ROTAT(0, 2) * EUV[2] - ROTAT(0, 0) * ROTAT(1, 2) * EUV[1] -
                                  ROTAT(1, 0) * ROTAT(0, 2) * EUV[1] + ROTAT(1, 0) * ROTAT(1, 2) * EUV[0]);
  GG(2, 1) = GG(2, 1) + DETEUV * (ROTAT(0, 1) * ROTAT(0, 2) * EUV[2] - ROTAT(0, 1) * ROTAT(1, 2) * EUV[1] -
                                  ROTAT(1, 1) * ROTAT(0, 2) * EUV[1] + ROTAT(1, 1) * ROTAT(1, 2) * EUV[0]);
  GG(2, 2) = GG(2, 2) + DETEUV * (ROTAT(0, 2) * ROTAT(0, 2) * EUV[2] - 1.0 * ROTAT(0, 2) * ROTAT(1, 2) * EUV[1] +
                                  ROTAT(1, 2) * ROTAT(1, 2) * EUV[0]);
  return GG;
}
...
const SymMatrix2x2& TrackState::positionCovarMatrix() const {
  if (!_PositionCovarValid) {
    ...
    /*double factor1 = -pow(_V(1,4),2) + _V(1,1)*_V(4,4) + s*sin(_Init.theta())*(2*_V(1,4)*_V(2,4) - 2*_V(1,2)*_V(4,4) +
    s*(-pow(_V(2,4),2) + _V(2,2)*_V(4,4))*sin(_Init.theta())); double factor2 = pow(_V(1,4),2) - _V(1,1)*_V(4,4) +
    s*sin(_Init.theta())*(-2*_V(1,4)*_V(2,4) + 2*_V(1,2)*_V(4,4) + s*(pow(_V(2,4),2) -
    _V(2,2)*_V(4,4))*sin(_Init.theta()));
    //std::cout << "V" << _V << std::endl;
    //std::cout << "f1,2: " << factor1 << " " << factor2 << std::endl;
    _InversePositionCovarMatrix(0,0) = (pow(_Init.invR(),4)*(_V(1,1) + s*sin(_Init.theta())*(-2*_V(1,2) +
    s*_V(2,2)*sin(_Init.theta()))))/factor1; _InversePositionCovarMatrix(0,1) = (pow(_Init.invR(),2)*(-_V(1,4) +
    s*_V(2,4)*sin(_Init.theta())))/factor2;
    //(1,0) filled by symmettry
    _InversePositionCovarMatrix(1,1) = _V(4,4)/factor1;
    */
    /*double factor = -pow(pow(_Init.invR(),2)*_V(0,1) - _V(1,4) + s*(-(pow(_Init.invR(),2)*_V(0,2)) +
    _V(2,4))*sin(_Init.theta()),2) + (pow(_Init.invR(),4)*_V(0,0) - 2*pow(_Init.invR(),2)*_V(0,4) + _V(4,4))*(_V(1,1) +
    s*sin(_Init.theta())*(-2*_V(1,2) + s*_V(2,2)*sin(_Init.theta()))); _InversePositionCovarMatrix(0,0) =
    (pow(_Init.invR(),4)*(_V(1,1) + s*sin(_Init.theta())*(-2*_V(1,2) + s*_V(2,2)*sin(_Init.theta()))))/factor;
    _InversePositionCovarMatrix(0,1) = (pow(_Init.invR(),4)*(-_V(0,1) + _V(1,4)/pow(_Init.invR(),2) + s*(_V(0,2) -
    _V(2,4)/pow(_Init.invR(),2))*sin(_Init.theta())))/factor;
    //(1,0) filled by symmettry
    _InversePositionCovarMatrix(1,1) = (pow(_Init.invR(),4)*(_V(0,0) + (-2*pow(_Init.invR(),2)*_V(0,4) +
    _V(4,4))/pow(_Init.invR(),4)))/factor;
    */
...
void TrackState::debugOut() {
  std::cout << "Pos:" << this->position().x() << "," << this->position().y() << "," << this->position().z()
            << std::endl;
  //		std::cout << "Mom:" << this->momentum().x() << "," << this->momentum().y() << "," <<
  // this->momentum().z() << std::endl;
