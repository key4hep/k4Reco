...
<br>TrackState objects define a point in space on the track, movement along the path of the track is performed by the
swimming methods of TrackState. For details of the parameterisation see Track
...
public:
  ...
  */
  ...
  inline bool sameTrack(TrackState* const Track) const { return (this->parentTrack() == Track->parentTrack()); }

  //! Is this the same track as Track?
  inline bool sameTrack(Track* const Track) const { return (this->parentTrack() == Track); }

  //! Distance from the TrackStates current position to Point
  inline double distanceTo(const Vector3& Point) const { return this->position().distanceTo(Point); }

  //! Distance squared from the TrackStates current position to Point
  inline double distanceTo2(const Vector3& Point) const { return this->position().distanceTo2(Point); }

  //! Distance in the XY plane from the TrackStates current position to Point
  inline double xyDistanceTo(const Vector3& Point) const { return this->position().subtract(Point).mag(RPhi); }
  ...
  inline double phi() const { return std::fmod(_Init.phi() + (_DistanceSwum * (_Init.invR())), 2.0 * 3.141592654); }

  //! Charge
  inline double charge() const { return _Charge; }

  //! Is this track neutral
  inline bool isNeutral() const { return (fabs(_Charge) < 0.000001); } // Floating point, hence no comparison to zero

  //! Is this track charged
  inline bool isCharged() const { return !this->isNeutral(); }
  ...
  inline Track* parentTrack() const { return _ParentTrack; }
...
inline std::basic_ostream<charT, traits>& operator<<(std::basic_ostream<charT, traits>& os, const TrackState& ts) {
  os << "TrackState: " << ts.parentTrack()->trackingNum() << " @ " << ts.position() << " C" << ts.isCharged()
     << " H:" << ts.parentTrack()->helixRep() << " err" << ts.parentTrack()->covarianceMatrix() << std::endl;
  return os;
}
} // namespace vertex_lcfi
#endif // TRACKSTATE_H
