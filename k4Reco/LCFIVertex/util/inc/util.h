  ...
  */
  ...
  public:
    ...
    bin<T>& operator++() {
      _contents++;
      return *this;
    } // increase the number of elements in this bin
  protected:
    T _regionLow{};
    T _regionHigh{};
    double _contents;
  ...
  template <class T>
  std::vector<bin<T>> histogram_data<T>::_binData(int numberOfBins, T minimum, T maximum, bool cumulate,
                                                  bool reverseCumulate) {
    ...
    for (int a = 0; a < numberOfBins; ++a) {
      ...
      if (cumulate && reverseCumulate)
        currentBin.contents() = totalData - binContents;
      else
        currentBin.contents() = binContents;
  ...
  std::vector<std::pair<double, double>> efficiency_purity<T>::eff_pur(int numberOfPoints) {
    ...
    std::vector<bin<T>> binnedSignal =
        _signal.binned_reversecumulative_data(numberOfPoints + 1, 0, 1); //, minimum, maximum );
    std::vector<bin<T>> binnedBackground =
        _background.binned_reversecumulative_data(numberOfPoints + 1, 0, 1); // minimum, maximum );
