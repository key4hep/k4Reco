...
namespace ZVTOP {
  ...
  bool pDecreasing(const TState& lhs, const TState& rhs) {
    return (fabs(lhs.track()->helixRep().invR()) < fabs(rhs.track()->helixRep().invR()));
  }
  ...
  void VertexFitterKalman::fitVertex(const std::vector<TrackState*>& Tracks, InteractionPoint* IP, Vector3& Result,
    ...
    if (Tracks.size() < 2) {
      ...
      if (m_useManualSeed)
        fitterLSM.setSeed(m_manualSeed);
      std::map<TrackState*, double> ChiSquaredOfTracks;
      double ChiSquaredOfIP;
      fitterLSM.fitVertex(Tracks, IP, Result, ResultError, ChiSquaredOfFit, ChiSquaredOfTracks, ChiSquaredOfIP);
      fP[0] = Result.x();
      fP[1] = Result.y();
      fP[2] = Result.z();
    ...
    for (std::vector<TState>::iterator it = fStates.begin(); fStates.end() != it; it++) {
      ...
      for (int iter = 0; iter < maxIter; iter++) {
        ...
        double mS[6];
        {
          double mSi[6] = {ffC[0] + mV[0], ffC[1] + mV[1], ffC[2] + mV[2],
        ...
        mCHt1[3] = ffC[7] - mV[7];
        mCHt2[3] = ffC[8] - mV[8];
        mCHt0[4] = ffC[10] - mV[10];
        mCHt1[4] = ffC[11] - mV[11];
        mCHt2[4] = ffC[12] - mV[12];
        mCHt0[5] = ffC[15] - mV[15];
        mCHt1[5] = ffC[16] - mV[16];
        mCHt2[5] = ffC[17] - mV[17];
  ...
  void VertexFitterKalman::fitVertex(const std::vector<TrackState*>& Tracks, InteractionPoint* IP, Vector3& Result,
                                     ...
                                     double& ChiSquaredOfIP) {
    Matrix3x3 ResultError;
    this->fitVertex(Tracks, IP, Result, ResultError, ChiSquaredOfFit, ChiSquaredOfTrack, ChiSquaredOfIP);
  }
  ...
  void VertexFitterKalman::fitVertex(const std::vector<TrackState*>& Tracks, InteractionPoint* IP, Vector3& Result,
    ...
    ChiSquaredOfIP = 0;
    if (IP)
      ChiSquaredOfIP = IP->chi2(Result);
  }

  // -----------------------------------------------------------------------------------

  double VertexFitterKalman::getDeviationFromVertex(const TState* state, const double v[], const double Cv[]) const {
    ...
    double sigmaS =
        0.1 + 10. * sqrt((d[0] * d[0] + d[1] * d[1] + d[2] * d[2]) / (mP[3] * mP[3] + mP[4] * mP[4] + mP[5] * mP[5]));

    double h[3] = {mP[3] * sigmaS, mP[4] * sigmaS, mP[5] * sigmaS};

    double mSi[6] = {mC[0] + h[0] * h[0], mC[1] + h[1] * h[0], mC[2] + h[1] * h[1],
