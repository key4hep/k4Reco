...
namespace ZVTOP {
  ...
  CandidateVertex::CandidateVertex(const std::vector<TrackState*>& Tracks, VertexFunction* VertexFunction,
                                   VertexFitter* Fitter, VertexResolver* Resolver, VertexFuncMaxFinder* MaxFinder)
      : _Fitter(Fitter), _Resolver(Resolver), _MaxFinder(MaxFinder), _IP(0), _TrackStates(Tracks),
        _VertexFunction(VertexFunction), _VertexFuncMaxIsValid(0), _FitIsValid(0), _ErrorOfFitIsValid(0) { /*NO OP*/ }

  // Construct from tracks,ip and vertex function
  CandidateVertex::CandidateVertex(const std::vector<TrackState*>& Tracks, InteractionPoint* IP,
                                   VertexFunction* VertexFunction, VertexFitter* Fitter, VertexResolver* Resolver,
                                   VertexFuncMaxFinder* MaxFinder)
      : _Fitter(Fitter), _Resolver(Resolver), _MaxFinder(MaxFinder), _IP(IP), _TrackStates(Tracks),
        _VertexFunction(VertexFunction), _VertexFuncMaxIsValid(0), _FitIsValid(0), _ErrorOfFitIsValid(0) { /*NO OP*/ }

  CandidateVertex::CandidateVertex(const Vector3& Position, const Matrix3x3& PositionError, double ChiSquaredOfFit,
                                   std::map<TrackState*, double> ChiSquaredOfTrack, double ChiSquaredOfIP)
      : _Position(Position), _PositionError(PositionError), _ChiSquaredOfFit(ChiSquaredOfFit),
        _ChiSquaredOfTrack(ChiSquaredOfTrack), _ChiSquaredOfIP(ChiSquaredOfIP), _FitIsValid(1),
        _ErrorOfFitIsValid(1) { /*NO OP*/ }

  CandidateVertex::CandidateVertex(const std::vector<CandidateVertex*>& Vertices, VertexFitter* Fitter,
                                   VertexResolver* Resolver, VertexFuncMaxFinder* MaxFinder)
      : _Fitter(Fitter), _Resolver(Resolver), _MaxFinder(MaxFinder), _VertexFuncMaxIsValid(0), _FitIsValid(0),
  ...
  bool CandidateVertex::removeTrack(Track* const TrackToRemove) {
    ...
    for (iTrack = _TrackStates.begin(); iTrack != _TrackStates.end() && ((*iTrack)->parentTrack() != TrackToRemove);
         ++iTrack)
      ;
  ...
  void CandidateVertex::mergeCandidateVertex(const CandidateVertex* SourceVertex) {
    ...
    for (std::vector<TrackState*>::iterator iSourceTrack = SourceList.begin(); iSourceTrack != SourceList.end();
         ++iSourceTrack) {
  ...
  void CandidateVertex::claimTracksFrom(const std::list<CandidateVertex*>& LosingVertices) {
    ...
    for (std::list<CandidateVertex*>::const_iterator iLosingVertex = LosingVertices.begin();
         iLosingVertex != LosingVertices.end(); ++iLosingVertex) {
      // Only one IP so if we have it they can't
      if (_IP)
        (*iLosingVertex)->removeIP();
      // Loop over my tracks, messaging to remove from losing vertex
      for (std::vector<TrackState*>::iterator iTrackState = _TrackStates.begin(); iTrackState != _TrackStates.end();
           ++iTrackState) {
  ...
  int CandidateVertex::trimByChi2(const double Chi2Threshold, VertexFitter* Fitter) {
    int NumRemoved = 0;
    do {
      // Refit now to make sure we have a fit that used the fitter specified, other wise chiSquaredOfTrack will invoke
      // default!
  ...
  void CandidateVertex::invalidateFit() const {
    _FitIsValid = 0;
    _ErrorOfFitIsValid = 0;
    // this->invalidateFuncMax(); //Taken out as we always keep the first max or one that is larger that it is replaced
    // with when clustering
  ...
  void CandidateVertex::refit(bool CalculateError) const {
    if (CalculateError) {
      _Fitter->fitVertex(this->trackStateList(), this->interactionPoint(), _Position, _ChiSquaredOfFit,
                         _ChiSquaredOfTrack, _ChiSquaredOfIP);
    } else {
      _Fitter->fitVertex(this->trackStateList(), this->interactionPoint(), _Position, _PositionError, _ChiSquaredOfFit,
                         _ChiSquaredOfTrack, _ChiSquaredOfIP);
    }
    _FitIsValid = 1;
  ...
  void CandidateVertex::refit(VertexFitter* Fitter, bool CalculateError) const {
    if (CalculateError) {
      Fitter->fitVertex(this->trackStateList(), this->interactionPoint(), _Position, _ChiSquaredOfFit,
                        _ChiSquaredOfTrack, _ChiSquaredOfIP);
    } else {
      Fitter->fitVertex(this->trackStateList(), this->interactionPoint(), _Position, _PositionError, _ChiSquaredOfFit,
                        _ChiSquaredOfTrack, _ChiSquaredOfIP);
    }
    _FitIsValid = 1;
    _ErrorOfFitIsValid = CalculateError;
  ...
  bool CandidateVertex::isResolvedFrom(CandidateVertex* const Vertex, const double Threshold,
                                       CandidateVertex::eResolveType Type) const {
    ...
    case FittedPosition:
      ...
      break;
    case NearestMaximum:
      return _Resolver->areResolved(this->vertexFuncMaxPosition(), Vertex->vertexFuncMaxPosition(), _VertexFunction,
                                    Threshold);
      break;
    }
  ...
  bool CandidateVertex::isResolvedFrom(CandidateVertex* const Vertex, const double Threshold,
                                       CandidateVertex::eResolveType Type, VertexResolver* /*Resolver*/) const {
    // Todo null vertex pointer check
    switch (Type) {
    case FittedPosition:
      return _Resolver->areResolved(this->position(), Vertex->position(), _VertexFunction, Threshold);
  ...
  double CandidateVertex::maxChiSquaredOfTrackIP() const {
    ...
    for (std::vector<TrackState*>::const_iterator iTrackState = _TrackStates.begin(); iTrackState != _TrackStates.end();
         ++iTrackState) {
      if (this->chiSquaredOfTrack(*iTrackState) > HighChiSquared)
        HighChiSquared = this->chiSquaredOfTrack(*iTrackState);
    }
