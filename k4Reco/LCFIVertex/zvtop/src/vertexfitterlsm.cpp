...
namespace ZVTOP {
  VertexFitterLSM::VertexFitterLSM() : _UseManualSeed(false), _InitialStep(100.0 / 1000.0) {}
  void VertexFitterLSM::fitVertex(const std::vector<TrackState*>& Tracks, InteractionPoint* IP, Vector3& Result) {
    ...
    // TODO - Is this the optimal seed? - For Vertexes we have added or removed tracks from the old fit maybe a good
    // start....
      ...
      if (Tracks.size() > 1) {
        ...
        for (int OuterIndex = 0; OuterIndex < N - 1; ++OuterIndex) {
          for (int InnerIndex = OuterIndex + 1; InnerIndex < N; ++InnerIndex) {
            ...
            Vector3 MidPoint = Tracks[OuterIndex]->position() +
                               ((Tracks[InnerIndex]->position() - Tracks[OuterIndex]->position()) * 0.5);
    ...
    // if (Tracks.size()>1)
    {
      ...
      // std::cout << Tracks.size() << " " << Position.mag()*10000 << " " <<  Position.distanceTo(Seed)*10000 <<
      // std::endl;
    } else
      Result = Seed;
  }
  ...
  void VertexFitterLSM::setInitialStep(double Step) { _InitialStep = Step; }

  double VertexFitterLSM::_chi2Contribution(const Vector3& point, TrackState* pTrackState) {
    if (0 == pTrackState)
      return 0;
    else
      return pTrackState->chi2(point);
  }
