...
namespace ZVTOP {
  VertexFunctionClassic::VertexFunctionClassic(std::vector<Track*>& Tracks, const double Kip, const double Kalpha,
  ...
  VertexFunctionClassic::VertexFunctionClassic(std::vector<Track*>& Tracks, InteractionPoint* IP, const double Kip,
                                               const double Kalpha, const Vector3& JetAxis) {
    _Kip = Kip;
    _Kalpha = Kalpha;
    _JetAxis = JetAxis;
    for (std::vector<Track*>::iterator iTrack = Tracks.begin(); iTrack != Tracks.end(); ++iTrack) {
  ...
  double VertexFunctionClassic::valueAt(const Vector3& Point) const {
    ...
    if (SumOfTubes > 0)
      ...
      {
        double alpha =
            acos((dlong + 0.01) / sqrt(((dlong + 0.01) * (dlong + 0.01)) + (dtran - 0.005) * (dtran - 0.005)));
        return exp(-_Kalpha * alpha * alpha) *
               ((_Kip * IPValue) + SumOfTubes -
                (((_Kip * IPValue * IPValue) + SumOfSquaredTubes) / ((_Kip * IPValue) + SumOfTubes)));
      } else
        return (_Kip * IPValue) + SumOfTubes -
               (((_Kip * IPValue * IPValue) + SumOfSquaredTubes) / ((_Kip * IPValue) + SumOfTubes));
    else
      return 0;
  }
