...
namespace ZVTOP {
  VertexFinderClassic::VertexFinderClassic(const std::vector<Track*>& Tracks, InteractionPoint* IP,
                                           const Vector3& JetAxis, double Kip, double Kalpha, double TwoProngCut,
                                           double TrackTrimCut, double ResolverCutOff)
      : _TrackList(Tracks), _IP(IP), _Kip(Kip), _Kalpha(Kalpha), _JetAxis(JetAxis), _TwoProngCut(TwoProngCut),
        _TrackTrimCut(TrackTrimCut), _ResolverCutOff(ResolverCutOff) {}

  void VertexFinderClassic::addTrack(Track* const Track) { _TrackList.push_back(Track); }

  void VertexFinderClassic::setIP(InteractionPoint* const IP) { _IP = IP; }
  ...
  std::list<CandidateVertex*> VertexFinderClassic::findVertices() {
    using std::cout;
    using std::endl;
    clock_t start, pstart;
    int debug = 0;
    // Make vertex function
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
      cout << "Constructing Vertex Function.....";
      cout.flush();
      pstart = clock();
    }
    start = clock();
    ...
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
      cout << "\tdone!\t\t\t" << ((double)clock() - (double)pstart) * 1000.0 / CLOCKS_PER_SEC << "ms" << endl;
      cout.flush();
    }
    ...
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
      cout << "2-Prong Creation.....";
      cout.flush();
      pstart = clock();
    }
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug < 0)
      std::cout << std::endl;
      ...
      for (int InnerIndex = OuterIndex + 1; InnerIndex < N; ++InnerIndex) {
        ...
        /*ofstream case2file ("chi2track.txt", ofstream::out | ofstream::app);
                if (case2file.is_open())
                {
                        ...
                        case2file << Track1->position().distanceTo(Track2->position()) << " " << CV->chiSquaredOfFit()
           << std::endl;
                }*/
        if (CV->maxChiSquaredOfTrackIP() <= _TwoProngCut && CV->vertexFuncValue() > 0.001) {
          CVList.push_back(CV);
          /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug < 0)
            std::cout << "2-prong of:" << TrackStates[OuterIndex]->parentTrack()->trackingNum() << ","
                      << TrackStates[InnerIndex]->parentTrack()->trackingNum() << " @ " << CV->position() << std::endl;
        } else {
          /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug < 0)
            std::cout << "-";
        }
      ...
      for (int Index = 0; Index < N; ++Index) {
        ...
        /*ofstream case2file ("chiip.txt", ofstream::out | ofstream::app);
                if (case2file.is_open())
                {
                        ...
                        case2file << Track->position().distanceTo(_IP->position()) << " " << CV->chiSquaredOfFit() << "
           " << Track->parentTrack()->helixRep() << std::endl;
                }*/
        ...
        if (CV->maxChiSquaredOfTrackIP() <= _TwoProngCut) {
          CVList.push_back(CV);
          /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug < 0)
            std::cout << "2-prong of:" << "IP" << "," << TrackStates[Index]->parentTrack()->trackingNum() << " @ "
                      << CV->position() << std::endl;
        } else {
          /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug < 0)
            std::cout << "-";
        }
    ...
    // And add one that is just the IP if we didn't add any IP-track vertices in the loop above - ensures we have a ip
    // object Commented out as FORTRAN doesn't add IP back in till before chi cut
    ...
    */
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
      cout << "\tdone!" << " " << CVList.size() << " Vertices" << "\t"
           << ((double(clock()) - double(pstart)) / CLOCKS_PER_SEC) * 1000 << "ms" << endl;
      cout.flush();
    }
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug > 1) {
    ...
    /*////////////////////////////////////////////////////////DEBUGLINE*/ // if (debug>1) {for
                                                                          // (std::list<CandidateVertex*>::iterator iCV
                                                                          // = CVList.begin();iCV != CVList.end();++iCV)
                                                                          // cout << **iCV <<endl;}
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
      cout << "Track removal based on clustering.....";
      cout.flush();
      pstart = clock();
    }
    ...
    for (std::vector<Track*>::iterator iTrack = _TrackList.begin(); iTrack != _TrackList.end(); ++iTrack) {
      ...
      for (std::list<CandidateVertex*>::iterator iCV = CVList.begin(); iCV != CVList.end(); ++iCV) {
        if ((*iCV)->hasTrack(*iTrack))
          AssocCVs.push_back(*iCV);
      }
      if (AssocCVs.empty()) {
        continue;
      }
      ...
      for (std::list<CandidateVertex*>::iterator iCV = AssocCVs.begin(); iCV != AssocCVs.end(); ++iCV) {
        bool resolved = true;
        for (std::list<CandidateVertex*>::iterator iRetainedCV = RetainedCVs.begin(); iRetainedCV != RetainedCVs.end();
             ++iRetainedCV) {
    ...
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
      cout << "Remove IP from all but highest V(r).....";
      cout.flush();
      pstart = clock();
    }
    ...
    /// std::cout << "5";
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
      cout << "Clustering by V(r)Max resolution.....";
      cout.flush();
      pstart = clock();
    }
    ...
    if (!CVList.empty()) {
      ...
      do {
        ...
        do {
          ...
          for (iCVertex = Cluster.begin(); iCVertex != Cluster.end(); ++iCVertex) {
            ...
            for (std::list<CandidateVertex*>::iterator iCVertex2 = Cluster.begin(); iCVertex2 != Cluster.end();
                 ++iCVertex2) {
      ...
      /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
        cout << "\tdone!" << "\t\t\t" << ((double(clock()) - double(pstart)) / CLOCKS_PER_SEC) * 1000 << "ms" << endl;
        cout.flush();
      }
      /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug > 1) {
        for (std::list<CandidateVertex*>::iterator iCV = CVList.begin(); iCV != CVList.end(); ++iCV)
    ...
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
      cout << "Vertex find complete" << "\t" << (double(clock()) - double(start)) / CLOCKS_PER_SEC << "s" << " "
           << ((double(clock()) - double(start)) * 1000) / (CLOCKS_PER_SEC * _TrackList.size() * _TrackList.size())
           << "ms per track^2" << endl;
      cout.flush();
    }
