...
namespace ZVTOP {
  ...
  void VertexFinderGhost::addTrack(Track* const Track) { _TrackList.push_back(Track); }

  void VertexFinderGhost::setIP(InteractionPoint* const IP) { _IP = IP; }
  ...
  std::list<CandidateVertex*> VertexFinderGhost::findVertices() {
    ...
    if (_TrackList.empty()) {
      if (debug)
        std::cout << "Ghost: No Tracks" << std::endl;
      if (!_IP)
    ...
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
      cout << "Initial Ghost direction finding and resize....";
      cout.flush();
      pstart = clock();
      start = clock();
    }
    ...
    Track* GhostTrack =
        GhostFinderStage1().findGhost(_InitialGhostWidth, _MaxChi2Allowed, _SeedDirection, _TrackList, _IP);
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
    ...
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
      cout << "Generating 1-prong Candidate Vertices + IP....";
      cout.flush();
      start = clock();
    }
    ...
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
      cout << "\tdone!" << " " << Candidates.size() << " Candidates" << "\t"
           << ((double(clock()) - double(start)) / CLOCKS_PER_SEC) * 1000 << "ms" << endl;
      cout.flush();
    }
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug > 1) {
      for (std::list<CandidateVertex*>::iterator iCV = Candidates.begin(); iCV != Candidates.end(); ++iCV)
        cout << *iCV << endl;
    }

    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
      cout << "Generating Trial Merged Vertices....";
      cout.flush();
      start = clock();
    }
    ...
    for (std::list<CandidateVertex*>::const_iterator iOuterCV = Candidates.begin(); iOuterCV != --(Candidates.end());
         ++iOuterCV) {
      std::list<CandidateVertex*>::const_iterator CopyiOuterCV = iOuterCV;
      for (std::list<CandidateVertex*>::const_iterator iInnerCV = (++CopyiOuterCV); iInnerCV != Candidates.end();
           ++iInnerCV) {
    ...
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
      cout << "\t\tdone!" << " " << TrialMergedCandidates.size() << "T " << Candidates.size() << "C Verts" << "\t"
           << ((double(clock()) - double(start)) / CLOCKS_PER_SEC) * 1000 << "ms" << endl;
      cout.flush();
    }
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug > 1) {
      for (std::list<CandidateVertex*>::iterator iCV = TrialMergedCandidates.begin();
           iCV != TrialMergedCandidates.end(); ++iCV)
        cout << *iCV << endl;
    }
    do {
      /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
        cout << "Find most probable trial vertex...";
        cout.flush();
        start = clock();
      }
      ...
      for (std::list<CandidateVertex*>::const_iterator iCV = TrialMergedCandidates.begin();
           iCV != TrialMergedCandidates.end(); ++iCV) {
      ...
      /*////////////////////////////////////////////////////////DEBUGLINE*/ if (0) {
        cout << "Promote trial to candidate and add new trials...";
        cout.flush();
        start = clock();
      }
      if (HighestProb > _MinimumProbability) {
        ...
        // We then need to remove the verticies the most probable one was composed of from the candidates list as they
        // are now represented in the candidates in merged form. We also need to remove the trial vertices that contain
        // any of the vertices that the most probable one was composed of
        for (std::vector<CandidateVertex*>::const_iterator iOriginalCV =
                 VerticesContainedIn[MostProbableVertex].begin();
             iOriginalCV != VerticesContainedIn[MostProbableVertex].end(); ++iOriginalCV) {
          Candidates.remove(*iOriginalCV);
          for (std::list<CandidateVertex*>::iterator iTrialCV = TrialMergedCandidates.begin();
               iTrialCV != TrialMergedCandidates.end();
               /*++iTrialCV*/) {
            // If we find that this Trial Vertex contained the Origial Candidate
            if (find(VerticesContainedIn[*iTrialCV].begin(), VerticesContainedIn[*iTrialCV].end(), *iOriginalCV) !=
                VerticesContainedIn[*iTrialCV].end()) {
    ...
    /*////////////////////////////////////////////////////////DEBUGLINE*/ if (debug) {
      cout << "Vertex find complete" << "\t" << (double(clock()) - double(pstart)) / CLOCKS_PER_SEC << "s" << endl;
      cout.flush();
    }
  ...
  Track* VertexFinderGhost::lastGhost() const { return _LastGhost; }
