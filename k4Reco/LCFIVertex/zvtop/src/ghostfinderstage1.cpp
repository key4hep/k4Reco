...
namespace ZVTOP {
  ...
  Track* GhostFinderStage1::findGhost(double InitialWidth, double MaxChi2Allowed, const Vector3& JetDir,
  ...
  void GhostFinderStage1::_fillLZeroChis() {
    ...
    for (std::vector<TrackState*>::const_iterator iTrack = _JetTracks.begin(); iTrack != _JetTracks.end(); ++iTrack) {
      ...
      _ChiToLZero.insert(
          std::pair<TrackState*, double>((*iTrack), (2 * ((*iTrack)->chi2(VertexPos) + IP.chi2(VertexPos)))));
    }
  }
  ...
  double GhostFinderStage1::_findAdjustedWidth(TrackState* GhostTrack, double CurrentWidth,
                                               std::vector<TrackState*>& JetTracks, double MaxChi2Allowed) {
...
/*double f;
                if (MaxChiOfFit > MaxChi2Allowed)
                {
                        ...
                        if (case2file.is_open())
                        {
                                ...
                                //	case2file << w*w << " " << 1.0/ChiOfFit2<< std::endl;//" " << VertexPos2.x() <<
   " " << VertexPos2.y() << " " << VertexPos2.z() << std::endl; case2file << w*w << " " << 1.0/ChiOfTracks2[&GhostTS2]
   << " " << 1.0/ChiOfTracks2[HiChiTrack]<< " " << 1.0/ChiOfFit2<< std::endl;//" " << VertexPos2.x() << " " <<
   VertexPos2.y() << " " << VertexPos2.z() << std::endl;
                                        //case2file << VertexPos2.x() << std::endl << VertexPos2.y() << std::endl <<
   VertexPos2.z() << std::endl;
                                }
                        }
                        std::cin >> f;
                }*/
