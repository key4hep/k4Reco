...
namespace ZVTOP {
  ...
  The perticular instance of these algorithms to be used by each candidate vertex is specified at construction, the
  constuctor defaults to fallback algorithms if none are specified. <br> In order to minimise the number of times a
  vertex is fitted, this only happens when needed. When a candidate vertex is created or its track content changed a
  flag is set (_FitIsValid) to 0, a function such as position() will check the status of this flag and fit the vertex if
  needed, so that a fit only occurs if the track content changed <i>and</i> the fit is needed. The vertex function
  maximum works in a similar way, except it is not currently invalidated on a change of track content as classic ZVTOP
  does not require this. <br> The per-instance algorithms can be overridden as the functions that use them are
  overloaded with a version that takes a pointer to the algorithm class. For example calling isResolvedFrom(Vertex,
  Threshold, Resolver) rather than isResolvedFrom(Vertex,Threshold). For fitting and max finding this is slightly
  complicated by the caching system mentioned above. For example to use a different fitter than that specified at
  constuction to get the position of the vertex, one must call refit(Fitter) and then position().
  ...
  */
  ...
  public:
    /*! This typedef determines the VertexFitter used if none is specified in the CandidateVertex constuctor. If changed
     * you may need to change the includes in the cpp file*/
    typedef VertexFitterLSM FallbackVertexFitter;
    /*! This typedef determines the VertexResolver used if none is specified in the CandidateVertex constuctor. If
     * changed you may need to change the includes in the cpp file*/
    typedef VertexResolverEqualSteps FallbackVertexResolver;
    /*! This typedef determines the VertexFuncMaxFinder used if none is specified in the CandidateVertex constuctor. If
     * changed you may need to change the includes in the cpp file*/
    ...
    As no fit or VertexFunction maximum is specifed at constuction the flags for these are set to invalid so that they
    are worked out when needed. Note defaults for fitter, resolver and max finder.
    \param Tracks A vector of pointers to the TrackState objects that form this CandidateVertex. The same TrackState can
    be given to multiple CandidateVertex objects, but this is may not desierable as the TrackState would then be swum
    ...
    \param Fitter Pointer to the VertexFitter that the vertex uses to fit itself. Defaults to the typedef
    FallbackVertexFitter.
    \param Resolver Pointer to the VertexResolver that the vertex uses to resolve itself from others. Defaults to the
    typedef FallbackVertexResolver.
    \param MaxFinder Pointer to the VertexFuncMaxFinder that the vertex uses to find the nearest VertexFunction maximum.
    Defaults to the typedef FallbackVertexFuncMaxFinder.
    */
    CandidateVertex(const std::vector<TrackState*>& Tracks, VertexFunction* VertexFunction,
                    VertexFitter* Fitter = _getFallbackFitter(), VertexResolver* Resolver = _getFallbackResolver(),
    ...
    Defaults to the typedef FallbackVertexFuncMaxFinder.
    */
    CandidateVertex(const std::vector<TrackState*>& Tracks, InteractionPoint* IP, VertexFunction* VertexFunction,
                    VertexFitter* Fitter = _getFallbackFitter(), VertexResolver* Resolver = _getFallbackResolver(),
                    VertexFuncMaxFinder* MaxFinder = _getFallbackMaxFinder());
    ...
    */
    CandidateVertex(const Vector3& Position, const Matrix3x3& PositionError, double ChiSquaredOfFit,
                    std::map<TrackState*, double> ChiSquaredOfTrack, double ChiSquaredOfIP);
    ...
    */
    CandidateVertex(const std::vector<CandidateVertex*>& Vertices, VertexFitter* Fitter = _getFallbackFitter(),
    ...
    */
    ...
    /*!
    This vertices TrackState list is searched for the first reference to TrackToRemove. If found it is removed and the
    fit invalidated via invalidateFit().
    ...
    */
    ...
    /*!
    This vertices TrackState list is searched for the first TrackState that has TrackToRemove as its ParentTrack. If
    found it is removed and the fit invalidated via invalidateFit().
    \param TrackToRemove Pointer to the TrackState to remove.
    ...
    */
    ...
    /*!
    Adds all the trackstates and the IP if held from SourceVertex to this vertices trackstate list, after removing any
    trackstates which have the same parent to avoid duplicates. The fit is automatically invalidated as this function
    calls addTrackState() If the vertex function maximum of the source vertex is bigger than this vertices, then it is
    also copied to this vertex. <br>SourceVertex remains unchanged.
    ...
    */
    ...
    /*!
    Message all the vertices in the list LosingVertices to remove trackstates that have the same parent as the
    trackstates in this vertex. The fits of the losing vertices are automatically invalidated as we call removeTrack().
    ...
    */
    ...
    //! Trim trackstates in order of decreasing chi squared until the vertex has a probabilty below that of the
    //! threshold
    /*!
    Using the fitter of this vertex (specified at constuction or default) the trackstate with the highest chi squared
    is removed if it is below threshold. If one was removed then refit and check again, repeating until we have reached
    thresold.
    ...
    */
    ...
    /*!
    Using the fitter of this vertex (specified at constuction or default) the trackstate with the highest chi squared
    is removed if it is above threshold. If one was removed then refit and check again, repeating until threshold is
    reached or we only have one track left.. Does not effect the IP held by this vertex if any.
    ...
    */
    ...
    inline void invalidateFuncMax() const { _VertexFuncMaxIsValid = 0; }

    //! Find the local vertex function maximum.
    /*!Find the nearest vertex function maximum using the vertexes _MaxFinder, using the fitted position of this vertex
    as the inital guess. Note this may lead to this vertex being refit as position() is called.
    */
    ...
    */
    ...
    \return 1 if the vertices are resolved, 0 otherwise.
    */
    bool isResolvedFrom(CandidateVertex* const Vertex, const double Threshold, eResolveType Type,
                        VertexResolver* Resolver) const;

    // Query Methods
    //! Return the TrackStates in this Vertex.
    /*!\return A Vector of pointers to the TrackStates in the vertex
     */
    inline const std::vector<TrackState*>& trackStateList() const { return _TrackStates; }
    ...
    */
    ...
    /*!
     */
    inline InteractionPoint* interactionPoint() const { return _IP; }
  ...
  private:
    ...
    inline friend std::basic_ostream<charT, traits>& operator<<(std::basic_ostream<charT, traits>& os,
                                                                const CandidateVertex& cv);
  };
  ...
  inline std::basic_ostream<charT, traits>& operator<<(std::basic_ostream<charT, traits>& os,
    ...
    if (cv.interactionPoint())
      os << "IP: " << cv.interactionPoint() << std::endl;
