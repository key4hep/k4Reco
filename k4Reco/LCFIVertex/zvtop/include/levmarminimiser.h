...
namespace ZVTOP {
  ...
  ZVTOP::Vector3 FunctionMinimiser<T>::Minimise(ZVTOP::Vector3 seedPoint, double muin, double vin) {
    ...
    // std::endl<< "Start Minimisation" << std::endl;
    std::cout << ".";
    std::cout.flush();
    Matrix3x3 I;
    I.clear();
  ...
  ZVTOP::Vector3 FunctionMinimiser<T>::_Jacobian(ZVTOP::Vector3 point) {
    ...
    // std::cout << _pFunc->valueAt(point + Vector3(delta,0,0)) << " " << _pFunc->valueAt(point + Vector3(0,delta,0)) <<
    // " " << _pFunc->valueAt(point + Vector3(0,0,delta)) << std::endl;
    return Vector3((_pFunc->valueAt(point + Vector3(delta, 0, 0)) - valueAtPoint) / delta,
                   (_pFunc->valueAt(point + Vector3(0, delta, 0)) - valueAtPoint) / delta,
                   (_pFunc->valueAt(point + Vector3(0, 0, delta)) - valueAtPoint) / delta);
  }
} // namespace ZVTOP
} // namespace vertex_lcfi
