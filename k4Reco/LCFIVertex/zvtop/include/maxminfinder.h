    ...
    do {
      ...
      do {
        ...
        if (mag > 0) {
          ...
          for (std::vector<double>::iterator iJacobian = jacobian.begin(); iJacobian < jacobian.end();
               ++iJacobian, ++iPoint) {
            (*iJacobian) = (*iPoint) - ((*iJacobian) / mag) * currentDelta;
          }
          // if (iterations > 10000) std::cout <<"Pos,jac,delt,chi: " << inspectionPoint[0] << " " << inspectionPoint[1]
          // << " " << inspectionPoint[2] << "    " << jacobian[0] << " " << jacobian[1] << " " << jacobian[2] << " "<<
          // currentDelta*10000 << " " << _pFunc->valueAt(inspectionPoint) << std::endl; if (inspectionPoint.size()==2)
          // std::cout <<"Pos,jac,delt,chi: " << inspectionPoint[0] << " " << inspectionPoint[1] << "    " <<
          // jacobian[0] << " " << jacobian[1]  << "     "<< currentDelta*10000 << " " <<
          // _pFunc->valueAt(inspectionPoint) << std::endl; if (inspectionPoint.size()==2) std::cout << iterations << "
          // " << _pFunc->valueAt(jacobian) << std::endl; If the step we are going to take takes us uphill then we have
          // found a minimum so break the inner loop and go to higher precision std::cout.flush();
...
//				if( !(_finite(inspectionPoint.x()) && _finite(inspectionPoint.y()) &&
//_finite(inspectionPoint.z())) )
#else
//				if( !(std::isfinite(inspectionPoint.x()) && std::isfinite(inspectionPoint.y()) &&
// std::isfinite(inspectionPoint.z())) )
#endif
          //					{
    ...
    std::cout << iterations << " :min at (" << inspectionPoint.x() << "," << inspectionPoint.y() << ","
              << inspectionPoint.z() << ") " << _pFunc->valueAt(inspectionPoint) << std::endl;
#endif
