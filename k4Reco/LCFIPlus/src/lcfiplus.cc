...
Globals* Globals::_theInstance = nullptr;
Globals* Globals::Instance() {
  if (_theInstance == nullptr)
    _theInstance = new Globals;
  return _theInstance;
}
Globals::Globals() : _bField(0.), _beamSizeX(0.), _beamSizeY(0.), _beamSizeZ(0.) {}
Globals::~Globals() { _theInstance = nullptr; }
...
// //		if(!IsExist(secvtxname,classname))throw(Exception("Event::getSecondaryVertices(): collection not
// found."));
...
//       double d0pull(1);
//       double z0pull(1);
//       for (vector<ErrorRescale>::const_iterator tabIter = _errRescaleTable.begin(); tabIter !=
//       _errRescaleTable.end(); ++tabIter) {
...
double Track::getZ() const { return _par[tpar::z0] + _par[tpar::om] * _flt; }
...
void Vertex::add(const Track* trk) { _tracks.push_back(trk); }
...
const Track* Vertex::getWorstTrack() const {
  if (_chi2Tracks.empty())
    return nullptr;

  double chi2max = 0;
  const Track* worstTrack = nullptr;
  for (const auto& [track, chi2] : _chi2Tracks) {
    if (chi2max < chi2) {
      worstTrack = track;
      chi2max = chi2;
    }
...
double Vertex::getPparallel(const TVector3& axis) const {
  ...
  for (const auto* track : getTracks())
    sum4v += *static_cast<const TLorentzVector*>(track);
  return sum4v.Vect().Dot(axis.Unit());
}

double Vertex::getVertexMass(const Vertex* daughter, const TVector3* paxis, const double dmass, double* ppt,
                             double* pp) const {
  const TVector3 axis = (paxis ? *paxis : getPos()).Unit();

  TLorentzVector sum4v;
  if (daughter) {
    TVector3 daughterDirection = daughter->getPos() - getPos();
    const double gammaBeta = daughterDirection.Mag() / 0.3 / 2.;
    const double momentumFromBoost = gammaBeta * dmass;
    const double momentumFromTracks = daughter->getPparallel(daughterDirection);
    daughterDirection.SetMag(momentumFromBoost > momentumFromTracks ? momentumFromBoost : momentumFromTracks);
    sum4v.SetVectM(daughterDirection, dmass);
  }

  for (const auto* track : getTracks())
    sum4v += *static_cast<const TLorentzVector*>(track);

  const double transverseMomentum = sum4v.Pt(axis);
  const double energy = sum4v.E() + transverseMomentum;
  const double longitudinalMomentum = sum4v.Vect().Dot(axis);
  if (ppt)
    *ppt = transverseMomentum;
  if (pp)
    *pp = longitudinalMomentum;

  return sqrt(energy * energy - longitudinalMomentum * longitudinalMomentum);
}
...
// double Vertex::getVertexMass(const Vertex* daughter, const TVector3* paxis, const double dmass, double* ppt, double*
// pp)const {
...
//   //cout << "getChi2TrackFit " << getCov()[0] << " " << getCov()[2] << " " << getCov()[5] << " " <<
//   tr->getCovMatrix()[tpar::d0d0] << " " << tr->getCovMatrix()[tpar::z0z0] << " " << tr->getCovMatrix()[tpar::omom] <<
//   " " << tr->getCovMatrix() [tpar::phph] << " " << tr->getCovMatrix()[tpar::tdtd] << " " << fitter.getChi2(this,tr)
//   << endl;
...
//       cout << "pi0 found!" << mcd->getPDG() << ", " << mcd->getParent()->getPDG() << ", " <<
//       mcd->getParent()->getParent()->getPDG() << endl;
//     }
//     it = mapmcp.find(mcp);
//     if (it == mapmcp.end())
//       mapmcp[mcp] = 1;
...
//     cout << "   [track] E = " << tr->E() << ", p = (" << tr->Px() << " " << tr->Py() << " " << tr->Pz() << "), chi2 =
//     " << getChi2Track(tr) << ", "; cout << "MCP = " << (mcp ? mcp->getPDG() : 0) << ", parent = " << (pmcp ?
//     pmcp->getPDG() : 0) << ", parent B = " << (pmcpb ? pmcpb->getPDG() : 0) << endl;
//     /*
...
//     if (!cl)throw(Exception(TString::Format("Parameters::remove: definition of class %s not found in TClass table:
//     cannot delete it.",_map[key].first->name())));
...
//       if (!cl)throw(Exception(TString::Format("Parameters::operator=: definition of class %s not found in TClass
//       table: cannot copy it.",type->name())));
...
//       if (!mc.IsValid())
//         throw(Exception(TString::Format("Parameters::operator=: class copy operator not found, class name = %s,
//         parameter type = %s", type->name(), (const char*)opetype)));
...
void Parameters::clear() {
  for (auto& [key, value] : _map) {
    const auto* type = value.first;
    void* data = value.second;
    if (type == &typeid(float))
      ...
      if (auto* klass = TClass::GetClass(*type))
        klass->Destructor(data);
    }
  }
  _map.clear();
}

Parameters& Parameters::operator=(const Parameters& reference) {
  if (this == &reference)
    return *this;
  clear();
  _allowstring = reference._allowstring;
  for (const auto& [key, value] : reference._map) {
    const auto* type = value.first;
    void* data = value.second;
    if (type == &typeid(double))
      add(key.c_str(), *static_cast<double*>(data));
    else if (type == &typeid(float))
      add(key.c_str(), *static_cast<float*>(data));
    else if (type == &typeid(long))
      add(key.c_str(), *static_cast<long*>(data));
    else if (type == &typeid(int))
      add(key.c_str(), *static_cast<int*>(data));
    else if (type == &typeid(bool))
      add(key.c_str(), *static_cast<bool*>(data));
    else if (auto* klass = TClass::GetClass(*type)) {
      void* copy = klass->New();
      TMethodCall assignment;
      const auto prototype = TString::Format("const %s &", klass->GetName());
      assignment.InitWithPrototype(klass, "operator=", prototype);
      if (!assignment.IsValid())
        throw Exception(TString::Format("Parameters::operator=: no copy operator for %s", type->name()));
      assignment.SetParamPtrs(&data, 1);
      assignment.Execute(copy);
      _map[key] = {type, copy};
    } else {
      throw Exception(TString::Format("Parameters::operator=: no ROOT class for %s", type->name()));
    }
  }
  return *this;
}
