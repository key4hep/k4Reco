...
GeometryHandler* GeometryHandler::Instance() {
  if (_theInstance == NULL)
    _theInstance = new GeometryHandler;
  return _theInstance;
}
// ctor/dtor
...
double Helix::Variance(const TVector3& p, double t) const {
  ...
  //  std::cout << "VARIANCE: " << t << ", " << variance << std::endl;
...
double Helix::VarianceDeriv(const TVector3& p, double t) const {
  ...
  int ret, ret2, ret3;
  double dvariance = ROOT::Math::Similarity(dif, derrXyz.Inverse(ret)) +
                     Dot(Transpose(errXyz.Inverse(ret2)) * dif, ddif) +
                     Dot(Transpose(errXyz.Inverse(ret3)) * ddif, dif);

  //		  std::cout << "DVARIANCE: " << t << ", " << dvariance << std::endl;
...
double Helix::VarianceDeriv2(const TVector3& p, double t) const {
  ...
  int ret, ret2, ret3;
  double ddvariance =
      Dot(Transpose(errXyz.Inverse(ret2)) * dddif, dif) + 2 * Dot(Transpose(derrXyz.Inverse(ret3)) * ddif, dif) +
      2 * ROOT::Math::Similarity(ddif, errXyz.Inverse(ret)) + ROOT::Math::Similarity(dif, dderrXyz.Inverse(ret)) +
      2 * Dot(Transpose(derrXyz.Inverse(ret3)) * dif, ddif) + Dot(Transpose(errXyz.Inverse(ret2)) * dif, dddif);

  //		  std::cout << "D^2(VARIANCE): " << t << ", " << dvariance << std::endl;
...
double Helix::LogLikelihood(const TVector3& p, double& tmin) const {
  ...
  //		  std::cout << "phi: " << phi << ", variance 1: " << t1 << ", " << v1 << ", variance 2: " << t2 << ", "
  //<< v2 << std::endl;
  ...
  if (GetFlag() == SECVTX) {
    // std::cerr << "This is for secondary vertex fitting." << std::std::endl;
    double tllimit = -1. / r;
    double tulimit = 1000. / r;
    min.SetLimitedVariable(0, "t", t, 1e-6, tllimit, tulimit);
    // todo: t<0 treatment
  } else if (GetFlag() == PRIVTX) {
    // std::cerr << "This is for primary vertex fitting." << std::std::endl;
  ...
  tmin = min.X()[0];
  if (verbose) {
    std::cout << "Minimizer result: p = ( " << p.x() << ", " << p.y() << ", " << p.z() << "), ";
    std::cout << std::scientific << "tmin = " << tmin << ", val = " << vf(&tmin) << std::fixed << std::endl;
  }

  double variance = vf(&tmin);

  //		  std::cout << "Minimizer result: " << b << ", p = ( " << p.x() << ", " << p.y() << ", " << p.z() << "),
  //"; 		  std::cout << "tmin = " << minimizer.XMinimum() << ", val = " << minimizer.FValMinimum() << std::endl;
...
Helix::Helix(const Track* trk, FITFLAG flag) : PointBase(flag) {
  ...
  */
  ...
  // std::  std::cout << _err << std::std::endl;
...
void Helix::GetPosErr(double t, SVector3& pos, SMatrixSym3& err, SMatrix53& trackToXyz) const {
  ...
  trackToXyz(2, 0) =
      -d0 * cos(phi0) - r * sin(phi0 - pi / 2 * _charge) - r * sin(-_charge * t + phi0 + pi / 2 * _charge); // dx/dphi0
  trackToXyz(2, 1) =
      -d0 * sin(phi0) + r * cos(phi0 - pi / 2 * _charge) + r * cos(-_charge * t + phi0 + pi / 2 * _charge); // dy/dphi0
...
void Helix::GetPosErrDeriv(double t, SVector3& dpos, SMatrixSym3& derr) const {
  ...
  // xyz error matrix
  SMatrix3 derrtmp = trackToXyzTranspose * _err * dtrackToXyz + dtrackToXyzTranspose * _err * trackToXyz;

  if (derrtmp(0, 1) - derrtmp(1, 0) > 1e-3 || derrtmp(0, 2) - derrtmp(2, 0) > 1e-3 ||
...
void Helix::GetPosErrDeriv2(double t, SVector3& ddpos, SMatrixSym3& dderr) const {
  ...
  // xyz error matrix
  SMatrix3 dderrtmp = trackToXyzTranspose * _err * ddtrackToXyz + ddtrackToXyzTranspose * _err * trackToXyz +
...
TVector3 Helix::ClosePoint(const Helix& hel) const {
  ...
  if (verbose) {
    std::cout << "xy closest point: (" << xc << ", " << yc << ")";
    if (nc == 2)
      std::cout << ", (" << xc2 << ", " << yc2 << ")";
    std::cout << std::endl;
  }
  ...
  if (verbose)
    std::cout << "z11: " << z11 << ", z12: " << z12 << ", z1: " << z1 << std::endl;
  ...
  if (nc == 2) {
    ...
    if (verbose)
      std::cout << "z21: " << z21 << ", z22: " << z22 << ", z2: " << z2 << std::endl;
...
double Helix::HelixLineDistance2Functor::operator()(const double* t) {
  ...
  //		  std::cout << "HelixLineDistance2Functor: t = " << t[0] << ", delta2 = " << delta2 << std::endl;
...
void Helix::HelixLineDistance2DerivFunctor::operator()(const double* t, double* output) {
  ...
  //		  std::cout << "HelixLineDistance2DerivFunctor: t = " << t[0] << ", deldelta2 = " << deldelta2 <<
  // std::endl;
...
TVector3 Helix::ClosePoint(const VertexLine& line, double* distance) const {
  ...
  //		  std::cout << "centerdistance = " << centerdistance << ", r = " << r << ", x0 = " << x0 << ", y0 = " <<
  // y0 << ", dir = " << dir.X() << ", " << dir.Y()
  //				<< ", costheta = " << costheta << ", deltaphi = " << (p0-p1).DeltaPhi(dir) << std::endl;

  TVector2 close2d[2];
  int nclosepoint = 1;

  TVector2 normal = dir.Rotate(TMath::Pi() / 2.);
  if ((p0 - p1).DeltaPhi(dir) < 0)
    normal *= -1;
  ...
  for (int n = 0; n < nclosepoint; n++) {
    ...
    //  std::cout << "closepoint2d: line: " << close2dline.X() << ", " << close2dline.Y() << ", " << zline;
    //  std::cout << " helix: " << close2d[n].X() << ", " << close2d[n].Y() << ", " << vr.z();
    //  std::cout << " distance: " << sqrt(delta2) << std::endl;
  ...
  //  std::cout << "3D close point selected: " << pos.x() << " " << pos.y() << " " << pos.z() << ", delta = " <<
  //  sqrt(mindelta2) << std::endl;
  ...
  pos = GetPos(minthelix2);
    std::cout << "3D point minimization result: " << pos.x() << " " << pos.y() << " " << pos.z() << ", delta = " <<
  sqrt(fret) << std::endl;
  */
  ...
  //  std::cout << "3D point minimization result2: " << pos.x() << " " << pos.y() << " " << pos.z() << ", delta = " <<
  //  sqrt(f(&minthelix2)) << std::endl;
...
double VertexLine::LogLikelihood(const TVector3& p, double& tmin) const {
  if (!_ip || !_vertex)
    return 0; // need error for loglikelihood
...
double Helix::LongitudinalDeviation(const Vertex* primary, const Vertex* secondary) {
  ...
  if (verbose)
    std::cout << "Helix::LongitudinalDeviation: variance = " << variance << ", pos = " << pos.x() << " " << pos.y()
              << " " << pos.z() << ", dist = " << ret << std::endl;
...
//		  std::cout << "Helix::LongitudinalDeviation: closepoint = " << closepoint.x() << " " << closepoint.y() << " " << closepoint.z() << ", d = " << dist << std::endl;
...
//		GetPosErr(t, xyz, errXyz);
//		  std::cout << "Helix::LongitudinalDeviation: 3d variance at vtxpoint = " << v << ", pos = " << xyz(0) << " " << xyz(1) << " " << xyz(2) << std::endl;
...
//		  std::cout << "Helix::LongitudinalDeviation: closepoint = " << closepoint.x() << " " << closepoint.y() << " " << closepoint.z() << ", d = " << dist << std::endl;
  v = LogLikelihood(closepoint,t);
  GetPosErr(t, xyz, errXyz);
//		  std::cout << "Helix::LongitudinalDeviation: 3d variance at closepoint = " << v << ", pos = " << xyz(0) << " " << xyz(1) << " " << xyz(2) << std::endl;
  ...
  if (verbose)
      std::cout << "Helix::LongitudinalDeviation: t = " << t << ", devmag = " << xyzT.Mag() << ", lmag " << vtxdir.Mag() << ", angle = " << xyzT.Angle(vtxdir) << std::endl;
...
// geometry operation ///////////////////////////////////////////////////////
double GeometryHandler::PointFit(const std::vector<PointBase*>& points, const TVector3& initial, Point* result) {
...
//		min.SetVariable(2,"z",initial.z(), 1e-6);
  ...
  if (!success) {
      std::cout << "minuit status: " << min.Status() << ", covmatrix status: " << min.CovMatrixStatus() << std::endl;
  }
  const double* xx = min.X();
  maxll = -pf(xx);
  ...
  */
  ...
  /*		for(int i=0;i<10 && !success;i++){
  			  std::cout << "Mimimizer status: " << min.Status() << ", Edm = " << min.Edm() << ", # tracks = " << points.size() << ", Tolerance = " << min.Tolerance()  << std::endl;
  			  std::cout << "Minimum: (" << xx[0] << ", " << xx[1] << ", " << xx[2] << "), l= " << maxll << std::endl;

  			  std::cout << "Minimization failed. retry # " << i+1 << std::endl;
  ...
  */
  ...
  if (verbose) {
      std::cout << "Mimimizer status: " << min.Status() << ", cov matrix status: " << min.CovMatrixStatus() << ", Edm = " << min.Edm();
      std::cout << ", # tracks = " << points.size() << ", Tolerance = " << min.Tolerance()  << std::endl;
      std::cout << "Minimum: (" << xx[0] << ", " << xx[1] << ", " << xx[2] << "), l= " << maxll << std::endl;
  }
  ...
  if (result) {
    ...
    for (int i=0; i<3; i++)for (int j=i; j<3; j++) {
        ...
        if (verbose)
            std::cout << "CovMatrix(" << i << "," << j << "): " << min.CovMatrix(i,j) << std::endl;
      }

    if (verbose) {
        std::cout << std::scientific;
        std::cout << "minuit result - covariant matrix:" << std::endl;
        std::cout << err(0,0) << "  " << err(0,1) << "  " << err(0,2) << std::endl;
        std::cout << err(1,0) << "  " << err(1,1) << "  " << err(1,2) << std::endl;
        std::cout << err(2,0) << "  " << err(2,1) << "  " << err(2,2) << std::endl;
        std::cout << std::fixed;
    }
  ...
  ROOT::Minuit2::FCNAdapter func([&f](const double* parameters) { return f(parameters); }, 1.);
  ...
  // if(status>0)   std::cout << "Minuit failed: status: " << status << std::endl;
  ...
  // verbose = true;
  if (verbose) {
    std::cout << "Minuit result: Minimum: (" << xx2[0] << ", " << xx2[1] << ", " << xx2[2] << "), l= " << maxll
              << std::endl;
  }
...
double GeometryHandler::HelixPointFit(const std::vector<Helix*>& helices, Point* result) {
  std::vector<PointBase*> points;
  ...
  for (unsigned int i = 0; i < helices.size() - 1; i++) {
    for (unsigned int j = i + 1; j < helices.size(); j++) {
      ...
      double ll = -pf(xyz);
      if (verbose)
        std::cout << "HelixPointFit: i = " << i << ", j = " << j << ", ll = " << ll << std::endl;
      if (maxll < ll) {
        maxllv = v;
        maxll = ll;
      }
  ...
  if (verbose)
    std::cout << "HelixPointFit: ll = " << maxll << " -> " << pfll << ", pos = (" << maxllv.x() << " " << maxllv.y()
              << " " << maxllv.z() << ") -> (" << pfpos.x() << " " << pfpos.y() << " " << pfpos.z() << ")" << std::endl;
  return pfll;
}
