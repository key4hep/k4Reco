...
double JetFinder::funcValencia(Jet& jet1, Jet& jet2, double /*Evis2*/, JetConfig& cfg) {
  ...
  TVector3 mom1 = jet1.Vect();
  TVector3 mom2 = jet2.Vect();
  double val = 2 * min(pow(e1, 2. * beta), pow(e2, 2. * beta)) *
               max(0., 1 - (mom1.Dot(mom2)) / (mom1.Mag() * mom2.Mag())) / pow(R_param, 2.);
  return val;
}

double JetFinder::funcVertex(Jet& jet1, Jet& jet2, double Evis2, JetConfig& cfg,
                             double (*func)(Jet&, Jet&, double, JetConfig&)) {
...
double JetFinder::funcKtBeamDistance(Jet& jet1, double /*Evis2*/, JetConfig& /*cfg*/) { return jet1.Pt() * jet1.Pt(); }
...
JetFinder::JetFinder(const JetConfig& cfg, const Vertex* ip)
    : _Yfunc(JetFinder::funcDurhamVertex), _YfuncBeam(JetFinder::funcDurhamBeamDistance), _cfg(cfg), _privtx(ip) {}
...
vector<Jet*> JetFinder::prerun(TrackVec& tracks, NeutralVec& neutrals, VertexVec& vertices_, int* nVertexJets) {
  ...
  if (_cfg.useMuonID) {
    ...
    for (unsigned int i = 0; i < tracks.size(); i++) {
      ...
      if (tr->E() < _cfg.muonIDMinEnergy)
        continue;

      if (((!_cfg.muonIDExternal &&
            algoEtc::SimpleSecMuonFinder(tr, 0., 0., 100., 0.05, 0., 1., 1.5, 5., 10., _privtx)) ||
           (_cfg.muonIDExternal && tr->getParticleIDProbability("muonProbability") > _cfg.muonIDMinProb)) &&
          (algoEtc::SimpleSecMuonFinder(tr, _cfg.muonIDMinD0Sig, _cfg.muonIDMinZ0Sig, _cfg.muonIDMaxDist, -1, -1, 10000,
                                        -1, 10000, 10., _privtx))) {
        ...
        if(tr->getMcp()){
          ...
          cout << "MUPDG: " << tr->getMcp()->getPDG() << " " << (tr->getMcp()->getSemiStableParent() ?
        tr->getMcp()->getSemiStableParent()->getPDG() : 0) << endl; const MCParticle *mcp = tr->getMcp(); while(mcp){
            cout << mcp->getPDG() << " ";
            mcp = mcp->getParent();
          }
          cout << endl;
        ...
        */
        // cout << "A fake vertex created by mu-candidate: " << tr->Px() << " " << tr->Py() << " " << tr->Pz() << " " <<
        // tr->E() << endl;
  ...
  for (unsigned int i = 0; i < vertices.size(); i++) {
    if (!usedVertices[i]) {
      ...
      for (unsigned int j = i + 1; j < vertices.size(); j++) {
        if (usedVertices[j])
          continue; // BUGFIX at 120413

        TVector3 pos2 = vertices[j]->getPos();

        // if(vertices[j]->getTracks().size() == 1){ cout << "Angle (" << j << ", " << i << ") : " << pos.Angle(pos2) <<
        // endl; }
  ...
  while (jets.size() > (unsigned int)_cfg.nJet) {
    // cout << "JetFinder: remained # jets with vertex is " << jets.size() << ", performing further combination..." <<
    // endl;
      ...
      for (unsigned int iv = 0; iv < jets[i]->getVertices().size(); iv++) {
        ...
        for (unsigned int j = i + 1; j < jets.size(); j++) {
          for (unsigned int jv = 0; jv < jets[j]->getVertices().size(); jv++) {
            ...
            double angle = pos.Angle(pos2) - (jets[j]->getVertices()[jv]->getTracks().size() == 1) *
                                                 (vertexcombineanglelepton - vertexcombineangle);
    ...
    if (jetToAssoc) {
      ...
      for (unsigned int k = 0; k < jetToAssoc->getVertices().size(); k++) {
        ...
        for (unsigned int n = 0; n < vtx->getTracks().size(); n++) {
          if (vtx->getTracks()[n]->Angle(tracks[i]->Vect()) < 1.e-6 &&
              (vtx->getTracks()[n]->E() - tracks[i]->E()) < 1.e-6) {
            veto_track = true;
            break;
          }
        }
...
vector<Jet*> JetFinder::run(vector<Jet*> jets, double* pymin, int ynjetmax) {
  ...
  while (njet > _cfg.nJet) {
    ...
    if (bmin != -1) {
      idel = bmin;
      // printf("jet %d removed to beam with Y=%e, p=%f,%f,%f, E=%f\n", bmin, Ymin, jets[bmin]->Px(), jets[bmin]->Py(),
      // jets[bmin]->Pz(), jets[bmin]->E());
    } else {
      if (imin > jmin) {
        int tmp = jmin;
        jmin = imin;
      ...
      //	     jets[imin]->Px(), jets[imin]->Py(), jets[imin]->Pz(), jets[imin]->E(), jets[jmin]->Px(),
      // jets[jmin]->Py(), jets[jmin]->Pz(), jets[jmin]->E());
...
Jet* convertJetVertex(const Jet* jet) {
  ...
  for (VertexVecIte iter = vertices.begin(); iter != vertices.end(); ++iter) {
    ...
    for (TrackVecIte trackIter = tracks2.begin(); trackIter != tracks2.end(); ++trackIter) {
      newjet->add(*trackIter);
    }
  }
