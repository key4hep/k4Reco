 ...
 */
#ifndef K4RECO_VERTEXFINDERSUEHARA_H
#define K4RECO_VERTEXFINDERSUEHARA_H 1
...
namespace VertexFinderSuehara {
  ...
  public:
    ...
    bool operator()(const Track* p1, const Track* p2) {
      //					double ipsig1 = sqrt(pow(trackD0Significance(p1, priVertex),2) +
      // pow(trackZ0Significance(p1,priVertex),2)); 					double ipsig2 =
      // sqrt(pow(trackD0Significance(p2, priVertex),2) + pow(trackZ0Significance(p2,priVertex),2));
      double ipsig1 = sqrt(pow(p1->getD0() / p1->getCovMatrix()[tpar::d0d0], 2) +
                           pow(p1->getZ0() / p1->getCovMatrix()[tpar::z0z0], 2));
      double ipsig2 = sqrt(pow(p2->getD0() / p2->getCovMatrix()[tpar::d0d0], 2) +
                           pow(p2->getZ0() / p2->getCovMatrix()[tpar::z0z0], 2));
  ...
  /*
                  ...
                  // track, chi-square threshold, maximum invmass, minimum impact parameter significance of tracks
     (either) vector<lcfiplus::Vertex*> * findCandidates(TrackVec &tracks, double chi2th, double massth, double
     ipsigth);

                  // find one vertex with build-up method
                  lcfiplus::Vertex* findOne(std::list<const Track *> &tracks, double chi2th, double massth, bool
     removeTracks);
                  // VertexFitterSimple version
                  lcfiplus::Vertex* findOne2(std::list<const Track *> &tracks, double chi2th, double massth, bool
     removeTracks);
  */
  // compare functions
  ...
  // obtain vertex list
  void GetVertexList(std::list<const Track*>& tracks, const Vertex* ip, std::vector<Vertex*>& vtx,
                     std::vector<Vertex*>& v0vtx, VertexFinderSueharaConfig& cfg);

  // // associating tracks to an existing vertex
  lcfiplus::Vertex* associateTracks(Vertex* vertex, const VertexVec& v0vtx, std::list<const Track*>& tracks,
                                    VertexFinderSueharaConfig& cfg, std::list<const Track*>* residualTracks = nullptr);
  void associateIPTracks(std::vector<Vertex*>& vertices, Vertex* ip, VertexFinderSueharaConfig& cfg);
  void associateIPTracksAVF(std::vector<Vertex*>& vertices, Vertex* ip, VertexFinderSueharaConfig& cfg);

  void buildUp(TrackVec& tracks, std::vector<Vertex*>& vtx, std::vector<Vertex*>& v0vtx, double chi2thpri,
               VertexFinderSueharaConfig& cfg, Vertex** ip = nullptr);
  // void buildUpForJetClustering(TrackVec& tracks, vector<Vertex*>& vtx);

  std::vector<Vertex*> makeSingleTrackVertices(VertexVec& vertices, TrackVec& tracks, VertexVec& v0vertices,
                                               const Vertex* ip, VertexFinderSueharaConfig& cfg);
  // vector<Vertex*> makeSingleTrackVertices(Jet* jet, TrackVec& tracks, VertexVec& v0vtx,
  //                                                  const Vertex* ip, VertexFinderSueharaConfig& cfg);

  void recombineVertices(std::vector<Vertex*>& vertices, std::vector<Vertex*>& singleVertices);
