 ...
 */
#ifndef K4RECO_VERTEXSELECTOR_H
#define K4RECO_VERTEXSELECTOR_H
...
public:
  ...
  VertexSelectorConfig()
      : rejectdist(false), minpos(0.), maxpos(1e+300), rejectdistnegative(false), rejectdistor(false), rejectk0(false),
        k0width(0.), k0dirdot(0.), rejectl0(false), l0width(0.), l0dirdot(0.), rejectconv(false), convmass(0.),
        convdist(0.), convdirdot(0.) {}
...
public:
  // const full version
  std::vector<const Vertex*> operator()(VertexVec& vertices, VertexSelectorConfig& config,
                                        std::vector<const Track*>& residualTracks, bool addTracks) {
      ...
      if (passesCut(vertices[i], config)) {
        ...
        if (!addTracks) {
          ...
          for (TrackVecIte itt = vertices[i]->getTracks().begin(); itt != vertices[i]->getTracks().end(); itt++) {
            std::vector<const Track*>::iterator itt2 =
                remove_if(residualTracks.begin(), residualTracks.end(),
                          bind(std::equal_to<const Track*>(), std::placeholders::_1, *itt));
  ...
  std::vector<Vertex*> operator()(const std::vector<Vertex*>& vertices, VertexSelectorConfig& config,
                                  std::vector<const Track*>& residualTracks, bool addTracks) {
  ...
  void operator()(std::vector<Vertex*>& vertices, VertexSelectorConfig& config) {
  ...
  bool isV0(const Vertex* vtx, const VertexSelectorConfig& cfg, const Vertex* primary) {
    ...
    if (cfg.rejectconv) {
      ...
      double convmassCorr =
          sqrt(vtx->getTracks()[0]->Vect().Mag() * vtx->getTracks()[1]->Vect().Mag() * (1 - cos(ang1 - ang2)));
  ...
  bool passesCut(const Vertex* vtx, const VertexSelectorConfig& cfg, const Vertex* primary = 0) {
    ...
    if (cfg.rejectdist) {
      ...
      if (cfg.rejectdistnegative)
        cond = !cond;
      if (cfg.rejectdistor && cond)
        return true;
      if (!cfg.rejectdistor && !cond)
        return false;
    }
    ...
    if (cfg.rejectk0 || cfg.rejectl0 || cfg.rejectconv) {
      if (isV0(vtx, cfg, primary))
        return false;
      //				if (isV0(vtx,cfg,primary)) {cout << "rejected by v0 selection" << endl;return
      // false;}
    }
