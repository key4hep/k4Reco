...
namespace tpar {
  ...
  enum cov { d0d0 = 0, d0ph, phph, d0om, phom, omom, d0z0, z0ph, z0om, z0z0, d0td, phtd, omtd, z0td, tdtd, covN };
...
template <class T>
std::vector<const T*>* constVector(std::vector<T*>* ptr) {
  return reinterpret_cast<std::vector<const T*>*>(ptr);
}
template <class T>
std::vector<const T*>& constVector(std::vector<T*>& ref) {
  return *reinterpret_cast<std::vector<const T*>*>(&ref);
}
template <class T>
const std::vector<const T*>* constVector(const std::vector<T*>* ptr) {
  return reinterpret_cast<const std::vector<const T*>*>(ptr);
}
template <class T>
const std::vector<const T*>& constVector(const std::vector<T*>& ref) {
  return *reinterpret_cast<const std::vector<const T*>*>(&ref);
}
...
// const vectors
typedef const std::vector<const Track*> TrackVec;
typedef const std::vector<const Neutral*> NeutralVec;
typedef const std::vector<const MCParticle*> MCParticleVec;
typedef const std::vector<const MCColorSinglet*> MCColorSingletVec;
typedef const std::vector<const Vertex*> VertexVec;
typedef const std::vector<const Jet*> JetVec;

// const iterators
typedef std::vector<const Track*>::const_iterator TrackVecIte;
typedef std::vector<const Neutral*>::const_iterator NeutralVecIte;
typedef std::vector<const MCParticle*>::const_iterator MCParticleVecIte;
typedef std::vector<const MCColorSinglet*>::const_iterator MCColorSingletVecIte;
typedef std::vector<const Vertex*>::const_iterator VertexVecIte;
typedef std::vector<const Jet*>::const_iterator JetVecIte;
...
class Parameters {
public:
  Parameters(bool as = true) : _map(), _allowstring(as) {}
  ...
  // fetch for non-std::vector
  template <typename T>
  void fetch(const char* key, T& ret, T def = T()) const {
    if (_map.find(key) == _map.end()) {
      std::cout << "Parameter " << key << " not found." << std::endl;
      ret = def;
    } else if (_map.find(key)->second.first == &typeid(T))
      ret = *(T*)(_map.find(key)->second.second);
    else if (_allowstring && _map.find(key)->second.first == &typeid(std::string)) {
      std::string type_test(*(std::string*)_map.find(key)->second.second);
      std::istringstream str(type_test.c_str());
      if (&typeid(T) == &typeid(bool) && (type_test == "true" || type_test == "false"))
        str >> std::boolalpha >> ret;
      else
        str >> ret;
    } else if (_allowstring && _map.find(key)->second.first == &typeid(std::vector<std::string>)) {
      std::string type_test((*(const std::vector<std::string>*)_map.find(key)->second.second)[0]);
      std::istringstream str(type_test.c_str());
      if (&typeid(T) == &typeid(bool) && (type_test == "true" || type_test == "false"))
        str >> std::boolalpha >> ret;
      else
      ...
      std::cout << "Parameter type invalid: key = " << key << ", type = " << _map.find(key)->second.first->name()
                << " vs " << typeid(T).name() << std::endl;
      throw(Exception("Parameter type invalid."));
    }
    return;
  }

  // fetch for std::vector
  template <typename T>
  void fetchArray(const char* key, std::vector<T>& ret) const {
    if (_map.find(key) == _map.end()) {
      std::cout << "Parameter " << key << " not found." << std::endl;
    } else if (_map.find(key)->second.first == &typeid(std::vector<T>))
      ret = *(std::vector<T>*)(_map.find(key)->second.second);
    else if (_map.find(key)->second.first == &typeid(T))
      ret.push_back(*(T*)(_map.find(key)->second.second));
    else if (_allowstring && _map.find(key)->second.first == &typeid(std::string)) {
      ret.push_back(T());

      std::istringstream str(*(std::string*)_map.find(key)->second.second);
      str >> ret[0];
    } else if (_allowstring && _map.find(key)->second.first == &typeid(std::vector<std::string>)) {
      const std::vector<std::string>* svec = (const std::vector<std::string>*)_map.find(key)->second.second;
      ret.resize(svec->size());
      for (unsigned int n = 0; n < svec->size(); n++) {
        std::istringstream str((*svec)[n]);
        str >> ret[n];
      }
    } else {
      std::cout << "Parameter type invalid: key = " << key << ", type = " << _map.find(key)->second.first->name()
...
public:
  // for non-std::vector only (if std::string parameter)
  template <typename T>
  ...
  template <typename T>
  void add(const char* key, T data) { // change T& data to T data for convenience
    if (_map.find(key) != _map.end())
      throw(Exception("Double entry."));

    _map[key] = std::pair<const std::type_info*, void*>(&typeid(T), new T(data));
  }
  ...
  void assign(const char* key, T data) {
    if (_map.find(key) == _map.end())
      throw(Exception("Parameters::assign(): the key has not been registered."));
    else if (_map.find(key)->second.first != &typeid(T))
      throw(Exception("Parameters::assign(): the value type is imcompatible."));
  ...
  const std::map<std::string, std::pair<const std::type_info*, void*>>& paramMap() const { return _map; }

private:
  void remove(const std::string& key, bool delmap);
  ...
  std::map<std::string, std::pair<const std::type_info*, void*>> _map;
  bool _allowstring;
};
...
public:
  ...
  virtual void init(Parameters* param) { _param = param; }
...
public:
  ...
  const std::vector<const Track*>& getTracks(const char* trackname = 0) const;
  const std::vector<const Neutral*>& getNeutrals(const char* neutralname = 0) const;
  const std::vector<const MCParticle*>& getMCParticles(const char* mcpname = 0) const;
  const std::vector<const MCColorSinglet*>& getMCColorSinglets(const char* mcpname = 0) const;
  const Vertex* getPrimaryVertex(const char* privtxname = 0) const;
  const std::vector<const Vertex*>& getSecondaryVertices(const char* secvtxname = 0) const;
  const std::vector<const Jet*>& getJets(const char* jetname = 0) const;
  ...
  std::vector<const MCParticle*> mcGetColorStrings() const;
  int mcNumberOfB() const;
  int mcNumberOfC() const;
  std::vector<const MCParticle*> mcGetSemiStableBs() const;
  std::vector<const MCParticle*> mcGetSemiStableCs() const;
  std::vector<const MCParticle*> mcGetSemiStableBCs(bool separatebc) const;
...
private:
  ...
  // members
  //      int id;
  //      std::vector<ErrorRescale> _errRescaleTable;
  //      void readErrorRescaleTable();
  //      bool _rescaleTableRead;

  std::string _defTrackName;
  std::string _defNeutralName;
  std::string _defMcpName;
  std::string _defPriVtxName;
  std::string _defSecVtxName;
  std::string _defJetName;
};
...
public:
  ...
  void setParticleIDProbability(std::string parName, double pidProbability) {
    _pidProbability[parName] = pidProbability;
  }

  double getParticleIDProbability(const char* parName) const {
    std::string partName = parName;
    double prob = 0.0;
  ...
  void setCorrEnergy(double mass) { _correnergy = sqrt(this->P() * this->P() + mass * mass); }

  double getCorrEnergy() const { return _correnergy; }
  ...
  // for storing BNess
  void setBNess(double bness) { _bness = bness; }

  double getBNess() const { return _bness; }

  double getCNess() const { return _cness; }

  // for storing CNess
  void setCNess(double cness) { _cness = cness; }
  double getX() const;
  double getY() const;
  double getZ() const;
...
private:
  ...
  std::map<std::string, double> _pidProbability;
  double _correnergy;
...
public:
  ...
  void setV0(bool v0 = true) { _isV0 = v0; }
...
public:
  ...
  // std::vector<MCParticle*> getParents() const;

  const std::vector<const MCParticle*>& getDaughters() const { return _dau; }
  ...
  const MCColorSinglet* getColorSinglet(const std::vector<const MCColorSinglet*>* pcs) const;
  ...
  std::vector<const lcfiplus::MCParticle*> promptTracks() const;
...
private:
  ...
  TVector3 _vertex;
  // list of daughters
  std::vector<const lcfiplus::MCParticle*> _dau;

  mutable const MCParticle* _dauForDecay; //! cached object

  // MC truth is not persisted by the EDM4hep vertexing path.  Keeping the
  // ROOT ClassDef here would require the legacy LCFIPlus dictionary, which is
  // deliberately not built by this port.
};
...
public:
  ...
  std::vector<const lcfiplus::MCParticle*> _initials;
  std::vector<const lcfiplus::MCParticle*> _finalstrings;
  std::vector<const lcfiplus::MCParticle*> _qqgluons;
  std::vector<const lcfiplus::MCParticle*> _finalcolorsinglets;
  std::vector<const lcfiplus::MCParticle*> _realparticles;
...
public:
  ...
  Vertex()
      : _id(-1), _chi2(0), _prob(0), _x(0), _y(0), _z(0), _isPrimary(false), _tracks(), _chi2Tracks(), _vertexingname(),
        _rvtxmom(), _pi0mom(), _rvtxmass(-1), _npi0(-1) {}
  Vertex(const double chi2, const double prob, const double x, const double y, const double z, const double cov[6],
         bool isPrim)
      : _id(-1), _chi2(chi2), _prob(prob), _x(x), _y(y), _z(z), _isPrimary(isPrim), _tracks(), _chi2Tracks(),
        _vertexingname(), _rvtxmom(), _pi0mom(), _rvtxmass(-1), _npi0(-1) {
    if (cov == 0) {
  ...
  Vertex(const Vertex& from)
      : _id(-1), _chi2(from._chi2), _prob(from._prob), _x(from._x), _y(from._y), _z(from._z),
        _isPrimary(from._isPrimary), _tracks(from._tracks), _chi2Tracks(from._chi2Tracks),
        _vertexingname(from._vertexingname), _rvtxmom(from._rvtxmom), _pi0mom(from._pi0mom), _rvtxmass(from._rvtxmass),
        _npi0(from._npi0) {
  ...
  const std::vector<const Track*>& getTracks() const { return _tracks; }
  const std::map<const lcfiplus::Track*, double>& getTracksChi2Map() const { return _chi2Tracks; }
  double getChi2Track(const Track* tr) const {
    std::map<const Track*, double>::const_iterator it = _chi2Tracks.find(tr);
    if (it != _chi2Tracks.end())
      return it->second;
    else
      return -1;
  }
  ...
  void setPrimary(bool isPrim) { _isPrimary = isPrim; }

  double getPparallel(const TVector3& axis) const;
  double getVertexMass(const Vertex* daughter = 0, const TVector3* paxis = 0, const double dmass = 1.87,
                       double* ppt = 0, double* pp = 0) const;
  ...
  // for AVF
  void setVertexingName(std::string vtxname) { _vertexingname = vtxname; }
  std::string getVertexingName() const { return _vertexingname; }
  ////////////////
...
private:
  ...
  std::vector<const lcfiplus::Track*> _tracks;
  std::map<const Track*, double> _chi2Tracks;

  // for AVF
  std::string _vertexingname;
...
public:
  ...
  const std::vector<const MCParticle*>& getDaughters() const { return _dauVec; }
  const std::vector<const Track*>& getRecoTracks() const { return _recoTrks; }
...
private:
  TVector3 _pos;
  const MCParticle* _parent;
  std::vector<const MCParticle*> _dauVec;

  std::vector<const Track*> _recoTrks;
...
public:
  ...
  const std::vector<const Track*>& getTracks() const { return _tracks; }
  const std::vector<const Neutral*>& getNeutrals() const { return _neutrals; }
  ...
  std::vector<const Vertex*> getVerticesForFT() const;
   ...
   */
  std::vector<const Track*> getAllTracks(bool withoutV0 = false) const;
  ...
  void add(const Vertex* vtx, bool removeTracks = true) {
    ...
    for (unsigned int i = 0; i < vtx->getTracks().size(); i++) {
      *(TLorentzVector*)this += *(TLorentzVector*)(vtx->getTracks()[i]);
      ...
      if (removeTracks) {
        std::vector<const Track*>::iterator it;
  ...
  static int sort_by_energy(const Jet* a, const Jet* b) { return (a->E() > b->E()); }
  ...
  void addParam(const char* paramname, Parameters& param) const {
    if (_params.count(paramname) == 1)
      throw(Exception("Jet::addParam: parameter of the specified name has been already registered."));
    _params[paramname] = param;
  }

  const Parameters* getParam(const char* paramname) const {
    if (_params.count(paramname) == 0)
      throw(Exception("Jet::getParam: parameter of the specified name is not registered."));
    return &(_params.find(paramname)->second);
  }

  const std::map<std::string, Parameters>& params() const { return _params; }

  void recalcFourMomentum() {
    TLorentzVector v;
    std::vector<const Track*> tr = getAllTracks();
...
private:
  ...
  std::vector<const Neutral*> _neutrals;
  std::vector<const Vertex*> _vertices;

  mutable std::map<std::string, Parameters> _params;
