...
LCCluster::LCCluster(double energy, double x, double y, double z, double weight,
    ...
    : _position{x, y, z}, _energy(energy), _weight(weight), _method(method), _theta(theta), _phi(phi),
...
std::ostream& operator<<(std::ostream& o, const LCCluster& rhs) {
  o << "  Energy " << std::setw(10) << rhs._energy << "  Method " << std::setw(4) << rhs._method << "  Weight "
    << std::setw(10) << rhs._weight << "  N Calo Hits " << std::setw(7) << rhs._caloHits.size() << "  pos(x,y,z) =  ( "
    << std::setw(10) << rhs._position[0] << " , " << std::setw(10) << rhs._position[1] << " , " << std::setw(10)
    << rhs._position[2] << " )"
...
void LCCluster::recalculatePositionFromHits(GlobalMethodsClass const& gmc) {
  ...
  for (auto const& calHit : _caloHits) {
    ...
    const double weight =
        GlobalMethodsClass::posWeight(calHit->getEnergy(), _energy, _method, logConstant) * rMin / rCell;
