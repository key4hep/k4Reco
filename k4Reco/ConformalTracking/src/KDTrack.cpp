...
double KDTrack::calculateChi2SZ(TH2F*, bool) {
  // Value to return
  double chi2 = 0.;
  ...
  for (auto it = m_clusters.begin() + 1; it != m_clusters.end(); ++it) {
    const auto& cluster = *it;
    // Get the global point details
    double xi     = cluster->getX();
...
void KDTrack::linearRegression(bool highPTfit) {
  ...
  for (const auto& cluster : m_clusters) {
    ...
    if (m_rotated) {
      double newx   = y;
      double newy   = -1. * x;
      double newer2 = cluster->getErrorU() * cluster->getErrorU();
      x             = newx;
      y             = newy;
      er2           = newer2;
    }
  ...
  std::array<std::array<double, 3>, 3> adjx{};
...
void KDTrack::linearRegressionConformal(bool) {
  ...
  m_chi2ndofZS = m_chi2ZS / (m_clusters.size() - 2);
}

// Mathematical sinc function
double KDTrack::sinc(double phi) const { return (sin(phi) / phi); }
